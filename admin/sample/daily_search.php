    ...
    <div class="content">
		...
		<table class="table-bordered">
	      <thead>
	      	...
	      	<tr>
	      		<th colspan="4">Daily Sales Report</th>
	      	</tr>
	      	...
	      	<tr>
	      		<th colspan="2" id="left-align">Date: <?php echo $date; ?></th>
	      ...
	      <tbody>
	        <?php 
	          	$result = mysqli_query($con, "SELECT products.product_id, products.prod_name, products.prod_price, sum(sales.quantity) as quantity, sum(sales.total_amount) as total  FROM products, sales WHERE (products.product_id=sales.product_id) and (sales.sales_date like '%$date%') and (sales.branch_id like '%$branch%') GROUP BY product_id");
	        ...
	        <tfoot>
	            <?php 
	              $result2 = mysqli_query($con, "SELECT sum(total_amount) as total FROM sales WHERE (sales_date like '%$date%') and (branch_id like '%$branch%')");
