...
<body>
    ...
    <div class="content">
		...
		<table class="table-bordered">
	      ...
	      <tbody>
	        <?php 
	          	$result = mysqli_query($con, "SELECT products.product_id, products.prod_name, products.prod_price, sum(sales.quantity) as quantity, sum(sales.total_amount) as total  FROM products, sales WHERE (products.product_id=sales.product_id) and (year(sales.sales_date)='$year') GROUP BY product_id");
	        ...
	        <tfoot>
	            <?php 
	              $result2 = mysqli_query($con, "SELECT sum(total_amount) as total FROM sales WHERE (year(sales.sales_date)='$year')");
