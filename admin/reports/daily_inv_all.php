    ...
    <div class="content">
    	<div id="content">
			...
			<table class="table-striped" id="salesreport">
			    ...
			    <tbody>
			        <?php 
			        	$result = mysqli_query($con, "SELECT branch1.pro_id, branch1.product_name, branch1.quantity, branch1.price, sum(sales_transac.quantity) as quantity2, sum(sales_transac.amount) as total FROM branch1 INNER JOIN sales_transac ON branch1.barcode = sales_transac.barcode INNER JOIN sales_activity ON sales_transac.invoice = sales_activity.invoice WHERE (sales_transac.sales_date like '%$date%') GROUP BY branch1.barcode");
			...
			<div class="row" id="row-foot">
		    	<?php 
		    		$result = mysqli_query($con, "SELECT sum(amount) as total FROM sales_activity WHERE (sales_date like '%$date%')");
