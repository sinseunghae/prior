    ...
    <div class="content">
    	<div id="content">
			...
			<table class="table-striped" id="salesreport">
		      ...
		      <tbody>
		        <?php 
		          	$result = mysqli_query($con, "SELECT product.product_id, product.product_name, product.product_price, sum(sales_transac.quantity) as quantity, sum(sales_transac.amount) as total FROM sales_activity INNER JOIN sales_transac ON sales_activity.invoice = sales_transac.invoice INNER JOIN product ON sales_transac.barcode = product.barcode WHERE (sales_activity.sales_date between '$date_from' and '$date_to') GROUP BY product.barcode");
