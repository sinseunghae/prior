<?php  
session_start(); 
if (empty($_POST['date_from'])) {$_POST['date_from']=$_SESSION['date_from'];}else{$_POST['date_from']=$_POST['date_from'];}
if (empty($_POST['branch'])) {$_POST['branch']=$_SESSION['branch'];}else{$_POST['branch']=$_POST['branch'];}
...
<body>
    ...
    <div class="content" id="content">
		...
		<table class="table-striped" id="salesreport">
		    ...
		    <tbody>
		        <?php 
		        	$result = mysqli_query($con, "SELECT product.product_id, product.product_name, product.product_price, sum(sales_transac.quantity) as quantity, sum(sales_transac.amount) as total FROM sales_activity INNER JOIN sales_transac ON sales_activity.invoice = sales_transac.invoice INNER JOIN product ON sales_transac.barcode = product.barcode WHERE (sales_activity.sales_date like '%$date%') and (sales_activity.Branch_ID like '%$branch%') GROUP BY product.barcode");
	...
	<div class="btn-options">
		<a href="daily_sales.php" style="text-decoration:none;">
