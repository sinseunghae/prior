    ...
    <div class="content">
		<div id="content">
			...
			<table class="table-striped" id="salesreport">
		      ...
		      <tbody>
		        <?php 
		        	$result = mysqli_query($con, "SELECT product.product_id, product.product_name, product.product_price, sum(sales_transac.quantity) as quantity, sum(sales_transac.amount) as total FROM sales_activity INNER JOIN sales_transac ON sales_activity.invoice = sales_transac.invoice INNER JOIN product ON sales_transac.barcode = product.barcode WHERE (year(sales_activity.sales_date)='$year') and (sales_activity.Branch_ID like '%$branch%') GROUP BY product.barcode");
